...
-- Production hardening for the existing 6.9 CDE. The directory RPC replaces
-- N-per-project document-picker fan-out with one permission-aware request.
...
as $$
  select d.id,d.company_id,d.project_id,d.site_id,d.folder_id,d.display_name,d.system_code,d.document_type,
    d.description,d.tags,d.state,d.current_version_id,d.version_count,d.created_by,d.created_at,d.updated_at,
    d.control_status,d.discipline,d.owner_user_id,d.review_due_at,d.approved_at,d.approved_by,
    v.version_number,v.version_label,v.original_filename,v.mime_type,v.size_bytes,v.upload_state,v.change_note,
    v.uploaded_by,v.finalized_at
...
-- Return truthful read-only/capability state for archived project/site contexts.
...
as $$
declare d public.documents%rowtype;v_cab uuid;v_operational boolean;
begin
  ...
  v_operational:=app_private.project_context_operational(d.project_id,d.site_id);
  return jsonb_build_object(
    'document',to_jsonb(d)-'search_vector',
    'project',(select jsonb_build_object('id',p.id,'code',p.code,'name',p.name,'status',p.status,'archived_at',p.archived_at) from public.projects p where p.id=d.project_id),
