...
const linkedOpenLoadCount=loadFilesCount;
await mod.handleAction('engineering-open-linked-document',{dataset:{id:'doc1'}});
assert.equal(loadFilesCount,linkedOpenLoadCount,'linked document open must not reload the whole Files workspace');assert.equal(openedDocument,'doc1');
...
assert.match(js,/routeDockHeight:Math\.max\(88/);assert.match(js,/engineering-pick-node-family/);assert.match(js,/localizedFolderName/);assert.match(js,/assetMime/);assert.doesNotMatch(js,/fixable=new Set\([^)]*orphan_node/);
