...
assert.match(app,/role-template-library-v2/);
assert.match(app,/clone-role-template/);
assert.match(app,/roleTemplates/);
...
for(const contract of ['Phase 5.2','.role-template-library-v2','.branding-preview','.member-profile-hero','.company-table-logo','.permission-groups-smart']) assert.ok(css.includes(contract));
