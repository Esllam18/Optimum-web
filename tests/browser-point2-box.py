...
captured={'actor':'owner','disabled_entitlements':set(),'client_role':None,'client_member':None,'settings':[],'platform_template':None,'platform_company':None,'storage_uploads':[],'saved_views':[],'bulk_calls':[],'work_settings':[],'work_calls':[],'workflow_calls':[],'pdc_calls':[],'global_actions_enabled':False,'platform_premium':False}
...
async def premium69_site_delivery_flow(browser):
 ...
 await page.locator(f'[data-action="open-claim-package"][data-id="{CLAIM}"]').click(); await page.locator('.claim360-hero-premium').wait_for(state='visible',timeout=5000)
 assert await page.locator('.claim-decision-card').count()==1
 assert await page.locator('.claim-lifecycle-premium > div').count()==4
 assert await page.locator('.claim-decision-metrics > article').count()==3
 assert await page.locator('.claim-evidence-list .claim-requirement').count()>=2
 assert '5' in await page.locator('.claim-decision-metrics > article').first.inner_text()
 assert await page.locator('[data-action="auto-collect-claim"]').count()==1
 ...
 drawer=page.locator('.drawer').last; box=await drawer.bounding_box(); assert box and box['width']<=392 and box['x']>=-2 and box['x']+box['width']<=392
 assert await page.evaluate('document.documentElement.scrollWidth <= window.innerWidth + 3')
 await page.screenshot(path='/mnt/data/optimum691-feature6-site-delivery-mobile-proof.png',full_page=True)
 await page.locator(f'[data-action="open-claim-package"][data-id="{CLAIM}"]').click(); await page.locator('.claim360-hero-premium').wait_for(state='visible',timeout=5000); await page.wait_for_timeout(220)
...
async def premium69_project_context_flow(browser):
 ...
 await initialize(page,'client','owner'); await page.route('https://client.test/**',local_route)
 html=(ROOT/'index.html').read_text().replace('<head>','<head><base href="https://client.test/"><script>location.hash="#/projects"</script>',1)
 await page.set_content(html,wait_until='domcontentloaded'); await page.locator('.pdc-portfolio-strip').wait_for(state='visible',timeout=8000)
 ...
 assert await page.locator('.project-site-list [data-action="open-site"]').count()==1
 ...
 assert await page.locator('.cabinet-folder-grid-primary .cabinet-folder-card').count()==6
 assert await page.locator('.cabinet-context-actions').inner_text() and ('Project work' in await page.locator('.cabinet-context-actions').inner_text() or 'عمل المشروع' in await page.locator('.cabinet-context-actions').inner_text())
 metrics=await page.evaluate('({scroll:document.documentElement.scrollWidth,inner:window.innerWidth})'); assert metrics['scroll']<=metrics['inner']+3
 ...
 await page.locator('[data-action="close-overlay"]').first.click(); await page.locator('[data-action="new-project"]').first.click()
...
async def premium69_cde_information_flow(browser):
 enable_pdc_contracts(); captured['actor']='owner'; captured['pdc_calls'].clear()
 # Mobile-first proof: context, folder browser, document control, storage, and recovery stay contained.
 page=await browser.new_page(viewport={'width':390,'height':844})
 await initialize(page,'client','owner'); await page.route('https://client.test/**',local_route)
 html=(ROOT/'index.html').read_text().replace('<head>','<head><base href="https://client.test/"><script>location.hash="#/projects"</script>',1)
 await page.set_content(html,wait_until='domcontentloaded'); await page.locator('.pdc-portfolio-hero').wait_for(state='visible',timeout=8000)
 await page.locator('.pdc-project-card [data-action="open-project-files"]').first.click(); await page.locator('.cde-workspace').wait_for(state='visible',timeout=6000)
 assert await page.locator('.cde-scope-bar').count()==1 and await page.locator('.cde-tools').count()==1 and await page.locator('.cde-view-switch').count()==1
 assert await page.locator('.cde-folder-browser-summary').is_visible()
 metrics=await page.evaluate('({scroll:document.documentElement.scrollWidth,inner:window.innerWidth})'); assert metrics['scroll']<=metrics['inner']+3, f'Premium CDE mobile files overflow: {metrics}'
 await page.locator(f'[data-action="open-folder"][data-id="{FOLDER}"]').first.click(); await page.locator('.cde-document-card').wait_for(state='visible',timeout=5000)
 assert float(await page.locator('.document-quick-actions').first.evaluate('(el)=>getComputedStyle(el).opacity'))>=0.99
 await page.locator(f'[data-action="open-document"][data-id="{DOC}"]').first.click(); drawer=page.locator('.drawer').last; await drawer.wait_for(state='visible',timeout=4000); await page.locator('.document-pulse-grid').wait_for(state='visible',timeout=4000); await page.wait_for_timeout(260)
 assert await page.locator('.document-pulse-grid>article').count()==5 and await page.locator('.document-control-panel').count()==1 and await page.locator('.document-related-panel').count()==1
 box=await drawer.bounding_box(); assert box and box['width']<=392 and box['x']>=-2 and box['x']+box['width']<=392
 metrics=await page.evaluate('({scroll:document.documentElement.scrollWidth,inner:window.innerWidth})'); assert metrics['scroll']<=metrics['inner']+3, f'Premium CDE mobile document overflow: {metrics}'
 await page.screenshot(path='/mnt/data/optimum691-feature3-document-mobile-proof.png')
 await page.locator('[data-action="close-overlay"]').first.click(); await page.locator('[data-action="open-storage-intelligence"]').click(); await page.locator('.storage-intelligence-summary').wait_for(state='visible',timeout=4000)
 assert await page.locator('.storage-decision-grid>article').count()==3
 await page.locator('[data-action="close-overlay"]').first.click()
 await page.evaluate("location.hash='#/trash'"); await page.locator('.trash-control').wait_for(state='visible',timeout=5000)
 assert await page.locator('.recovery-overview-strip>article').count()==4 and await page.locator('.recovery-item').count()>=2
 metrics=await page.evaluate('({scroll:document.documentElement.scrollWidth,inner:window.innerWidth})'); assert metrics['scroll']<=metrics['inner']+3, f'Premium CDE mobile recovery overflow: {metrics}'
 await page.screenshot(path='/mnt/data/optimum691-feature3-recovery-mobile-proof.png',full_page=True); await page.close()
 # Desktop proof: secondary card controls stay quiet until intent; workspace and Document 360 stay contained.
 page=await browser.new_page(viewport={'width':1500,'height':1000})
 await initialize(page,'client','owner'); await page.route('https://client.test/**',local_route); await page.set_content(html,wait_until='domcontentloaded'); await page.locator('.pdc-portfolio-hero').wait_for(state='visible',timeout=8000)
 await page.locator('.pdc-project-card [data-action="open-project-files"]').first.click(); await page.locator('.cde-workspace').wait_for(state='visible',timeout=6000); await page.locator(f'[data-action="open-folder"][data-id="{FOLDER}"]').first.click(); await page.locator('.cde-document-card').wait_for(state='visible',timeout=5000)
 assert float(await page.locator('.document-quick-actions').first.evaluate('(el)=>getComputedStyle(el).opacity'))<=0.01
 assert float(await page.locator('.folder-card-actions').first.evaluate('(el)=>getComputedStyle(el).opacity'))<=0.01
 metrics=await page.evaluate('({scroll:document.documentElement.scrollWidth,inner:window.innerWidth})'); assert metrics['scroll']<=metrics['inner']+3, f'Premium CDE desktop overflow: {metrics}'
 await page.locator(f'[data-action="open-document"][data-id="{DOC}"]').first.click(); await page.locator('.document-pulse-grid').wait_for(state='visible',timeout=4000)
 assert await page.locator('.document-version-history').count()<=1
 await page.screenshot(path='/mnt/data/optimum691-feature3-cde-desktop-proof.png',full_page=True); await page.close()
...
async def premium69_organization_access_flow(browser):
 ...
 assert await page.locator('.member360-tabs button').count()==4
 await page.locator('[data-action="orgos-member-tab"][data-tab="access"]').click(); await page.locator('[data-member360-panel="access"].active .member360-access-state').wait_for(state='visible',timeout=3000)
 access_box=await page.locator('[data-member360-panel="access"].active').bounding_box(); state_box=await page.locator('[data-member360-panel="access"].active .member360-access-state').bounding_box(); drawer_box=await page.locator('.drawer-body').bounding_box(); print('P2 BOX',access_box,state_box,drawer_box,flush=True); assert access_box and access_box['height']>=120, f'Member 360 access panel collapsed: {access_box}'
 await page.screenshot(path='/mnt/data/optimum-point2-team-member360-desktop.png',full_page=True)
 await page.locator('[data-action="close-overlay"]').first.click()
 # More filters include organization unit + access model; bulk appears only after selection.
 ...
 await page.locator('.orgos-unit-card').first.click(); await page.locator('.unit360-v2').wait_for(state='visible',timeout=4000)
 ...
 editable=page.locator('.role-capability-card [data-action="edit-role"]').first; await editable.click(); await page.locator('[data-role-impact]').wait_for(state='visible',timeout=4000)
 ...
 assert await page.locator('.settings-insight-grid').count()==0
 ...
 for route,ready in [('team','.team-directory-shell'),('organization','.orgos-chart'),('roles','.role-capability-list'),('settings','.settings-command-center')]:
  ...
  if route=='team':
   ...
   box=await page.locator('.drawer.premium-drawer').last.bounding_box(); assert box and box['x']>=6 and box['x']+box['width']<=384
...
async def premium69_global_actions_flow(browser):
 ...
 await page.locator('.project360-hero').wait_for(state='visible',timeout=7000)
 await page.locator('[data-action="close-overlay"]').first.click()
...
async def premium69_dashboard_flow(browser):
 ...
 assert await page.locator('.dashboard-decision-signal').count()>=3
 assert await page.locator('.dashboard-focus-row').count()>=1
 assert await page.locator('.dashboard-project-row').count()>=2
 ...
 assert await page.locator('.dashboard-activity-preview').count()==1
 assert await page.locator('.dashboard-decision-signal.tone-danger').count()>=1, 'overdue work must surface as a decision signal'
 ...
 await page.locator('[data-action="close-overlay"]').first.click()
 # Project pulse opens Project 360.
 await page.locator('.dashboard-project-row').first.click()
 await page.locator('.project360-hero').wait_for(state='visible',timeout=7000)
 await page.locator('[data-action="close-overlay"]').first.click()
 ...
 assert await page.locator('.dashboard-content-grid.single').count()==1
...
async def main():
 ...
 flows={'client':client_flow,'orgos':organization_os_flow,'limited':limited_permission_flow,'mobile':mobile_responsive_flow,'premium69':premium69_organization_access_flow,'premiumf2':premium69_project_context_flow,'premiumcde':premium69_cde_information_flow,'policy':adaptive_policy_flow,'platform':platform_flow,'platformmobile':platform_mobile_flow,'workos':work_os_flow,'worklimited':work_os_limited_flow,'excellence':work_excellence_flow,'workmobile':work_mobile_excellence_flow,'premiumwork':premium69_work_os_flow,'pdc':pdc_owner_flow,'pdclimited':pdc_limited_flow,'pdcmobile':pdc_mobile_flow,'site69':site69_owner_flow,'site69limited':site69_limited_flow,'site69mobile':site69_mobile_flow,'premiumsite':premium69_site_delivery_flow,'premiumglobal':premium69_global_actions_flow,'premiumdashboard':premium69_dashboard_flow,'premiumplatform':premium69_platform_final_flow,'foundationv2':premium_foundation_v2_flow,'foundationv2mobile':premium_foundation_v2_mobile_flow,'foundationv2responsive':premium_foundation_v2_responsive_flow,'foundationv2auth':premium_foundation_v2_auth_flow}
