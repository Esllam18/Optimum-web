...
const api={user:{email:'qa@optimum.local'},select:async(t)=>structuredClone(tables[t]||[]),rpc:async(name)=>{rpcCalls++;if(name==='save_engineering_draft_v2'||name==='save_engineering_draft')return{lock_version:4,saved_at:'2026-08-05T10:02:00Z'};return{};},createSignedUrl:async()=>'',uploadObject:async()=>{},deleteObject:async()=>{}};
...
api.rpc=async(name)=>{rpcCalls++;if(name==='save_engineering_draft_v2'||name==='save_engineering_draft')throw new Error('Revision changed by another user');return{};};
await mod.handleAction('engineering-save',{});
