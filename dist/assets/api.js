...
class OptimumApi {
  ...
  async request(path, { method = 'GET', body, headers = {}, retry = true } = {}) {
    // Refresh proactively before Data/Storage requests. Concurrent callers share the
    // same refreshPromise, avoiding bursts of avoidable 401s at token rollover.
    if (this.session?.access_token && retry) await this.ensureFreshSession();
